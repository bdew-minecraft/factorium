...
import net.bdew.lib.gui.{Point, Rect, Sprite}
...
case class WidgetMode[T <: Enumeration](p: Point, ds: DataSlotEnum[T], textures: Map[T#Value, Sprite], pktConstructor: T#Value => NetworkHandler.Message, locPfx: String) extends Widget {
  ...
  override def draw(m: PoseStack, mouse: Point, partial: Float): Unit = {
    parent.drawTexture(m, iconRect, textures(ds.value))
  }
