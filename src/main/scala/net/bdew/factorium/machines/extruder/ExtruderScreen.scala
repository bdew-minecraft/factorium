...
class ExtruderScreen(container: ExtruderContainer, playerInv: Inventory) extends BaseScreen(container, playerInv, container.te.getDisplayName) with SwitchableScreen[ExtruderContainer] with UpgradableScreen[ExtruderContainer] with SidedItemIOScreen[ExtruderContainer] {
  override def init(): Unit = {
    ...
    addMode(SwitchableContainer.NormalMode, ExtruderTextures.screen) { sub =>
      ...
      sub.add(new WidgetProgressBar(Rect(71, 35, 24, 16), ProcessingTextures.arrow, container.te.progress))
