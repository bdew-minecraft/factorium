...
class BaseMachineItem(block: BaseMachineBlock[_]) extends BlockItemKeepData(block, Items.props) {
  ...
  override def appendHoverText(stack: ItemStack, world: Level, tip: util.List[Component], flag: TooltipFlag): Unit = {
    ...
    if (stack.hasTag && stack.getTag.contains("data", Tag.TAG_COMPOUND)) {
      ...
      addTankTip(data,  "tank", tip)
      addTankTip(data,  "inputTank", tip)
      addTankTip(data,  "outputTank", tip)
  ...
  def addTankTip(data: CompoundTag, name: String, tip: util.List[Component]): Unit = {
    if (data.contains(name, Tag.TAG_COMPOUND)) {
      val stack = FluidStack.loadFluidStackFromNBT(data.getCompound(name))
      if (!stack.isEmpty) {
